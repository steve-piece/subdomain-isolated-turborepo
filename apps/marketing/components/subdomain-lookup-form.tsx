...
export function SubdomainLookupForm({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tenants.length > 0) {
        ...
        const exactSubdomainMatch = tenants.find((tenant) => 
          tenant.subdomain.toLowerCase() === searchValue.toLowerCase()
        )
        
        const exactNameMatch = tenants.find((tenant) => 
          tenant.name.toLowerCase() === searchValue.toLowerCase()
        )
    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enter your organization or company name</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex flex-col gap-6">
              <div className="grid gap-2 relative">
                ...
                <Input
                  ...
                  onFocus={() => setShowDropdown(searchResults.length > 0 || hasSearched)}
                  className="w-full"
                />
                      ...
                      <>
                        {searchResults.map((tenant) => (
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-500">{tenant.subdomain}.{process.env.NEXT_PUBLIC_APP_DOMAIN || 'yourapp.com'}</span>
                          </button>
                        ))}
                      </>
                    ) : hasSearched ? (
                ...
                <p className="text-sm text-muted-foreground">
                  We'll find your organization and take you to the right place
                </p>
              </div>
              
              {error && <p className="text-sm text-red-500">{error}</p>}
              
              <Button type="submit" className="w-full" disabled={isLoading || !searchTerm.trim()}>
